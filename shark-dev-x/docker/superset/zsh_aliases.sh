# Superset aliases (loaded for interactive shells)

# Activate Superset virtualenv and run Superset CLI commands easily
alias superset="source /opt/superset-venv/bin/activate && superset"
...
alias superset-worker-stop="pkill -f 'celery.*worker' || true"

# Show Superset logs (web server)
alias superset-logs="tail -f ~/superset.log || true"

# Show Celery worker logs
alias superset-worker-logs="tail -f ~/celery.log || true"
...
alias superset-venv="source /opt/superset-venv/bin/activate"
